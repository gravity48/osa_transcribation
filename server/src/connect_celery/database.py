...
class PostworkDB:
    ...
    @open_session
    def unmark_all_records(self, clean_comments: bool = False, session: Session = None):
    ...
    def read_records_list(self, period_to, period_from, options, limit=None, offset=0):
        ...
        request_basis = session.query(SPR_SPEECH_TABLE.s_inckey).filter(
            SPR_SPEECH_TABLE.s_datetime >= period_from,
